...
describe('Element.* commands: type', () => {
    ...
    it('types by sending keys, appending to the value, on element once it is present in DOM and visible', async () => {
        ...
        await browser.element('input').type('pressed by 18 keys');
        expect(await textOf('#counter')).toContain('18');
        expect(await valueOf('input')).toContain('The text was pressed by 18 keys');
    });
    ...
    it('types through then(perform.type) on element once it is present in DOM and visible', async () => {
        ...
        expect(await textOf('#counter')).toContain('0');
        await browser.element('input').perform(something).then(perform.type('pressed by 18 keys'));
        expect(await textOf('#counter')).toContain('18');
        expect(await valueOf('input')).toContain('The text was pressed by 18 keys');
    });

    it('on Configuration.typeByJs=true, appends to the value by js, on element once it is present in DOM and visible',
       async () => {
        await GIVEN.openedEmptyPageWithBody(`
            <label id="counter">0</label>
        `);
        ...
        expect(await textOf('#counter')).toContain('0');
        await browser.element('input', {typeByJs: true}).type('appended by js in 0 keys pressed');
        expect(await textOf('#counter')).toContain('0');
        expect(await valueOf('input')).toContain('The text was appended by js in 0 keys pressed');
    });
