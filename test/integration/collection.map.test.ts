...
describe('Collection map each to first element', () => {
    ...
    it('should not be found on creation', async () => {
        const collection = browser.all('.not-existing').map('span');
    ...
    it('search is postponed until asking actual element data like "has specific texts?"', async () => {
        const collection = browser.all('div').map('span');
        ...
        expect(await collection.get(their.size))
            .toBe(await webelements('div span:nth-child(1)').then(it => it.length));
    });
    ...
    it('is performed on each subsequent "ask"', async () => {
        ...
        `);
        expect(await collection.matching(have.texts('second', 'fourth'))).toBe(true);
    });
    ...
    it('waits for matched condition (like have.texts) in should', async () => {
        ...
        expect(await browser.all('div').map('span').waitUntil(have.texts('first', 'third')))
            .toBe(true);
    });
