...
describe('Collection filtered by condition', () => {
    it('should not be found on creation', async () => {
        const collection = base_1.browser.all('.not-existing').filteredBy(lib_1.have.cssClass('black-hole'));
    ...
    it('search is postponed until asking actual element data like "has specific texts?"', async () => {
        ...
        `);
        const collection = base_1.browser.all('li').filteredBy(lib_1.have.cssClass('will-exist'));
...
//# sourceMappingURL=collection.filteredBy.condition.test.js.map