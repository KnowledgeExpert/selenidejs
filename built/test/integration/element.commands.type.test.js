...
describe('Element.* commands: type', () => {
    it('types by sending keys, appending to the value, on element once it is present in DOM and visible', async () => {
        ...
        await base_1.browser.element('input').type('pressed by 18 keys');
        expect(await base_1.textOf('#counter')).toContain('18');
        expect(await base_1.valueOf('input')).toContain('The text was pressed by 18 keys');
    });
    it('types through then(perform.type) on element once it is present in DOM and visible', async () => {
        await base_1.GIVEN.openedEmptyPageWithBody(`
            <label id="counter">0</label>
        `);
        await base_1.GIVEN.executeScriptAfter(base_1.data.timeouts.smallest, `
        ...
        `);
        expect(await base_1.textOf('#counter')).toContain('0');
        await base_1.browser.element('input').perform(something).then(lib_1.perform.type('pressed by 18 keys'));
        expect(await base_1.textOf('#counter')).toContain('18');
        expect(await base_1.valueOf('input')).toContain('The text was pressed by 18 keys');
    });
    it('on Configuration.typeByJs=true, appends to the value by js, on element once it is present in DOM and visible', async () => {
        await base_1.GIVEN.openedEmptyPageWithBody(`
            <label id="counter">0</label>
        `);
        await base_1.GIVEN.executeScriptAfter(base_1.data.timeouts.smallest, `
        ...
        `);
        expect(await base_1.textOf('#counter')).toContain('0');
        await base_1.browser.element('input', { typeByJs: true }).type('appended by js in 0 keys pressed');
        expect(await base_1.textOf('#counter')).toContain('0');
        expect(await base_1.valueOf('input')).toContain('The text was appended by js in 0 keys pressed');
    });
    it('fails to type with error on not present in DOM element (after timeout)', async () => {
...
\tbrowser.element(By(css selector, input)).type: pressed by 18 keys
Reason:
\tno such element: Unable to locate element: {"method":"css selector","selector":"input"}`);
        });
    });
    it('fails to type with error on not visible element (after timeout)', async () => {
        await base_1.GIVEN.openedEmptyPage();
        await base_1.GIVEN.executeScriptAfter(base_1.data.timeouts.smallest, `
            $('body').append('<input style="display: none" value="The text was "></input>')
            $('input').keypress(function(e) {
...
//# sourceMappingURL=element.commands.type.test.js.map