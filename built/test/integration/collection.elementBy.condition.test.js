...
describe('Collection element by condition', () => {
    it('should not be found on creation', async () => {
        const collection = base_1.browser.all('.not-existing').elementBy(lib_1.have.cssClass('black-hole'));
    ...
    it('search is postponed until asking actual element data like "has specific text?"', async () => {
        ...
        `);
        const element = base_1.browser.all('li').elementBy(lib_1.have.cssClass('will-exist'));
...
\tbrowser.all(By(css selector, li)).elementBy(has css class 'will-exist').has text: Bob
Reason:
\tCannot find element by condition «has css class 'will-exist'» from webelements collection:
[<li>John</li>,<li id="queen">Mary</li>]`);
        });
    });
...
//# sourceMappingURL=collection.elementBy.condition.test.js.map