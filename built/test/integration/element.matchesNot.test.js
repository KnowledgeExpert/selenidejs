...
describe('Element.matchesNot as negated condition based "predicate"', () => {
    it('on element condition (like be.visible) that is not matched, returns true (no waiting)', async () => {
        ...
        expect(await (await base_1.webelement('button')).isDisplayed())
            .toBe(false);
        expect(await base_1.browser.element('button').matchesNot(lib_1.be.visible)).toBe(true);
        expect(await (await base_1.webelement('button')).isDisplayed())
            .toBe(false);
    });
    it('on element condition (like be.visible) that is matched, returns false (no waiting)', async () => {
        ...
        expect(await (await base_1.webelement('button')).isDisplayed())
            .toBe(true);
        expect(await base_1.browser.element('button').matchesNot(lib_1.be.visible)).toBe(false);
        expect(await (await base_1.webelement('button')).isDisplayed())
            .toBe(true);
    });
});
